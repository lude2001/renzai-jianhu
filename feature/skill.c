...
int query_same_type(object ob1,object ob2)
{

	if (ob1->query("magic/tz_type/skill") == ob2->query("magic/tz_type/skill") && ob1->query("magic/tz_type/attribute") == ob2->query("magic/tz_type/attribute"))
	return 1;

	return 0;
}

int query_tz(string skill)
{
	string *fy = ({"armor", "boots", "cloth", "head", "neck", "pants", "waist"});
	object obj,*objs = ({ });
	object me = this_object();

	foreach (string armor in fy)
	{
		if (!objectp(obj = me->query_temp("armor/" + armor)))
		return 0;
		
		if (obj->query("magic/tz_type/skill") != skill)
		return 0;

		objs += ({ obj });
	}

	objs = filter_array(objs,(: query_same_type :));

	if (sizeof(objs) < 7)
		return 0;
	else if (!me->query("item_tz/" + skill + "/number"))
	{
	    me->set("item_tz/" + skill + "/number",50 + random(51));
	}

	return me->query("item_tz/" + skill +"/number");
}
...
varargs int query_skill(string skill, int raw)
{
	...
	if (! raw)
	{
		...
		s += query("yijing/skill_add/" + skill);
