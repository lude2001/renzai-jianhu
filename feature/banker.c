...
return notify_fail("本服共有存款："+(balance/10000)+"两黄金！\n");
}
...
int do_deposit(string arg)						      
{
	...
	log_file("qianzhuang/cun", ctime(time()) + me->query("name") + "(" + me->query("id") + ")存入了" + amount + what_ob->query("name") + "。\n");
...
int do_withdraw(string arg)
{
	...
	if(ob->move(me))
	{
		me->start_busy(1);
		me->add("balance", -v);
	    log_file("qianzhuang/qu", ctime(time()) + me->query("name") + "(" + me->query("id") + ")取出了" + type + "。\n");
