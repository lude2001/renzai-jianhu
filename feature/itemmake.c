...
int apply_armor()
{
	...
	if (query("itemlevel"))
	p += p*query("itemlevel")/10;
	
	s = d + p;
	
	if (query("magic/tz_type"))
	s *= 2;
...
int inlay_gem(object obj,int place)
{
  if (!mapp(query("hole/"+place)))
  {
  set("hole/"+place+"/name",obj->query("can_be_inlay/name"));
  set("hole/"+place+"/skill",obj->query("can_be_inlay/skill"));
  set("magic/skill/"+obj->query("can_be_inlay/skill"),1);//设定存在此武功的宝石。
  obj->add_amount(-1);
  save();
  }
  return 1;  
...
string weapon_inlay()
{
 ...
 string msg;
 msg = HIR"【镶嵌情况】：\n"NOR;
 if (hole = query("magic/hole"))
 {
 i =1;
 while(i<=hole)
 {
  if (mapp(query("hole/"+i)))
   msg += HIY"〖孔位"+chinese_number(i)+"〗："NOR+query("hole/"+i+"/name")+"\n";
   else msg+=HIY"〖孔位"+chinese_number(i)+"〗：空\n"NOR;
   i++;
 }
}
return msg;
