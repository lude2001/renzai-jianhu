...
varargs void improve_level(object me)
{
	...
	{
		msg= ZJOBLONG+HIR "◎"HIY "侠行奇缘等级系统：" HIR "◎"NOR+ZJBR;
	    msg +=HIC"≡"HIY"─────────────────────"HIC"≡"NOR+ZJBR;
		msg +="你目前的等级为："+me->query("level")+ZJBR;
		msg +="你目前的经验为："+me->query("combat_exp")+ZJBR;
		msg +="你下一级所需经验为："+level_max_exp(me)+ZJBR;
		 msg +=HIC"≡"HIY"─────────────────────"HIC"≡"NOR+ZJBR;
		msg +="为你推荐以下方式提升经验\n";
		msg +=ZJOBACTS2+ZJMENUF(2,2,10,30);
		msg +="挂机任务:help questd/yamen"ZJSEP;
	//	msg +="黄蓉任务:help questd/huang"ZJSEP;
		//msg +="蛮荒大陆:fly manhuang"ZJSEP;
		msg +="师门任务:quest";
		write(msg+"\n");
	}   
}
...
varargs void up_tianfu(object me,string arg)
{
	...
	{
	string msg;	
msg= ZJOBLONG+HIR "◎"HIY "侠行奇缘属性系统(点击即可增加属性)：" HIR "◎"NOR+ZJBR;
msg +=HIC"≡"HIY"─────────────────────"HIC"≡"NOR+"\n";
msg+=ZJOBACTS2+ZJMENUF(2,2,10,30);
