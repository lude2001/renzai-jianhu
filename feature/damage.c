...
varargs int receive_damage(string type, int damage, object who)
{
	...
	if ((this_object()->query("gain/jianshang") || percent) && who)
	{
		...
		else if ((this_object()->query("meridian/足三阴经") >= 24 || this_object()->query("meridian/足三阳经") >= 24) && (jianshang * 20 < damage || percent))
		{

			if (this_object()->query("meridian/足三阴经") >= 24)
				percent += 10;

			if (this_object()->query("meridian/足三阳经") >= 24)
				percent += 10;
		...
		tell_object(this_object(), "触发减伤效果，抵消伤害" + jianshang + "。\n");
	}
	if (damage <= 0)
		damage = 1;
