...
{
	// 接受
	add_action("do_accept", ({"accept_letter_quest", "accept"}));

	// 拒绝
	add_action("do_refuse", ({"refuse_letter_quest", "refuse"}));
}
...
string long()
{
	...
	/*msg += HIC "如果你愿意接受此任务，请在" HIW + 
	       ...
	       HIC "秒之内：\n"ZJURL("cmds:accept_letter_quest quest")ZJSIZE(20)"确认"NOR"或者"ZJURL("cmds:refuse_letter_quest quest")ZJSIZE(20)"拒绝"NOR"\n";*/
	       msg += "如果你愿意接受此任务，请在"+ 
	       chinese_number(query("dest_time") - time()) +"秒之内：\n" + ((random(2))?(ZJURL("cmds:accept_letter_quest quest")ZJSIZE(20)"确认"+ NOR + "    " + ZJURL("cmds:refuse_letter_quest quest")ZJSIZE(20)"拒绝"):(ZJURL("cmds:refuse_letter_quest quest")ZJSIZE(20)"拒绝"+ NOR + "    " + ZJURL("cmds:accept_letter_quest quest")ZJSIZE(20)"确认")) + NOR + "\n";
	return msg;
}
string long2(object me)
{
	...
	// 想立即拒绝的话就输入 refuse，否则等 30 秒也没关系
	msg += "如果你愿意接受此任务，请在"+ 
	       chinese_number(query("dest_time") - time()) +"秒之内：\n" + ((random(2))?(ZJURL("cmds:accept_letter_quest quest")ZJSIZE(20)"确认"+ NOR + "    " + ZJURL("cmds:refuse_letter_quest quest")ZJSIZE(20)"拒绝"):(ZJURL("cmds:refuse_letter_quest quest")ZJSIZE(20)"拒绝"+ NOR + "    " + ZJURL("cmds:accept_letter_quest quest")ZJSIZE(20)"确定")) + NOR + "\n";
	return msg;
}
...
int do_accept(string arg)
{
	...
	if(me->is_ghost())
		return notify_fail("你先还阳再说。\n");
	
	if (me->query("shimen/lun")>=15)
  	{
    if ("adm/daemons/questd"->check_shimen("yanzhen",me))
  	return 1;
  	}
	if (me->query("id") != query("quester"))
