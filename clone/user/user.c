...
private void net_dead()
{
	...
	net_dead = 1;
	if (userp(this_object()) && ! query("doing")&&!(environment(this_object())&&(environment(this_object())->query("close_room")||environment(this_object())->query("work_room"))))
	{
...
int query_baoji()
{
	...
	if (query("gain/2ap"))
	baoji += query("gain/2ap");

	if (query_skill("liuhe-weizun"))
	{
		baoji += query_skill("liuhe-weizun") / 10;
	}
	
	if (query("titlea/baoji"))
	{
	     baoji += query("titlea/baoji");   
	}
...
//食物buff
int  food_call_out(function fun, int delay)
{
	call_out("food_function", delay, bind(fun, this_object()));
	return 1;
}

void food_function(function fun) { evaluate(fun); }

int full_chongmai()
{
    object me = this_object();
    	if(me->query("meridian/冲脉") >=24&& me->query("meridian/带脉") >=19&& me->query("meridian/阳维脉") >=24&& me->query("meridian/阴维脉") >=18&& me->query("meridian/阳跷脉") >=23&& me->query("meridian/阴跷脉") >=18&& me->query("meridian/足三阴经") >=24&& me->query("meridian/足三阳经") >=24&& me->query("meridian/左阴经") >=24&& me->query("meridian/左阳经") >=24&& me->query("meridian/手三阴经") >=24&& me->query("meridian/手三阳经") >=24)
	return 1;
