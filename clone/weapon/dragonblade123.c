...
{ 
     remove_call_out("return_to_xie"); 
     call_out("return_to_hong", 60); 
} 
...
{ 
     ...
     { 
             if (me->is_fight()) 
             { 
                     call_out("return_to_xie", 1); 
                     return; 
             } 

             message_vision("突然出现个黑衣人把你的屠龙刀抢走了。\n", me); 
     } else 
     { 
             message("vision", "忽然一个黑衣人捡起屠龙刀，叹了口" 
                               "气，摇摇头走了。\n", me); 
     } 
