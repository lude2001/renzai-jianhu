...
int do_effect(object me)
{
    ...
    if (time() / 86400 > me->query("cpfuben/wolong/time") / 86400)
    {
        me->delete("cpfuben/wolong");
    }
    
    if (me->query("cpfuben/wolong/number") > 10 *(1 +  (time()-1652168569) /  86400))
    {
        tell_object(me,"你今天的卧龙副本次数达到上限了。\n");
		return 1;
    }
	...
	if(FUBEN_D->create_fuben(me,query("fuben_type")))
	{
		...
		me->add("cpfuben/wolong/number",1);
		me->set("cpfuben/wolong/time",time());
	}
