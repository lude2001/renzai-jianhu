...
void auto_lian(object me,string skill,int times)
{
	...
	if(!me||!me->query_temp("pending/auto_lian"))
	    ...
	    if (me->query_temp("practicejb"))
    {
    remove_call_out(me->query_temp("practicejb"));//按照句柄清除
    me->delete_temp("practicejb");
    }
    
	if(me->is_busy())
	jb = call_out("auto_lian",1,me,skill,times);
	...
	jb = call_out("auto_lian",10,me,skill,times);
	}
