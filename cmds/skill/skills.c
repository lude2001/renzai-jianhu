...
int strlens(string str1,string str2);
int lfg(int exp);
...
//		return notify_fail("只有巫师或有师徒/夫妻/结拜/主宠/同盟关系的人才能察看他人的技能。\n");
	...
	str = ZJOBLONG + "目前可以支撑技能等级："+chinese_number(lfg(ob->query("combat_exp")))+"\n";
...
*/
	...
	if(!arg)
		str +=YEL"　查看准备:bei"ZJSEP"　取消准备:bei none"ZJSEP"  分类激发:fskill"ZJSEP+NOR;
	text = ({});
ftext = ({});
	...
	/*
		...
		pops = ZJPOPMENU;
			...
			if (wizardp(me))			
			pops += ZJSP2"设定"+to_chinese(sname[i])+"的名称|chinesed "+sname[i];
			pops += ZJSP2"查看"+to_chinese(sname[i])+"|chaski "+sname[i];
		}
		//无法sort，只能逆反
		if(member_array(sname[i], mapped)!=-1)
		text +=({ sprintf("%s%s%s%s"NOR ZJBR WHT"　%d/%d%%"NOR":%s",
			...
			ftext +=({ sprintf("%s%s%s%s"NOR ZJBR WHT"　%d/%d%%"NOR":%s",
			skcolor,
			(lrn[sname[i]] >= (skl[sname[i]]+1) * (skl[sname[i]]+1)) ? HIM : "",
			"□",
			skillname, lvl, percent,pops) });
	...
	text+=ftext;
//	text= sort_array(text, "strlens", this_object());
	if(!page) page = 1;
	all = sizeof(text)/30;
...
{
int i;
if (strsrch(str1,"■")!=-1&&strsrch(str2,"■")==-1)
i=1;
else
if (strsrch(str1,"■")==-1&&strsrch(str2,"■")==-1)
i=0;
