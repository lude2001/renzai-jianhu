...
int main(object me, string arg)
{
    ...
    if (me->query_temp("skill_jin") > time())
        return notify_fail("(你的丹田已被封闭无法使用绝技。\n)");
	...
	if (sscanf(arg, "%s and %s", arg, and) == 2 || sscanf(arg, "%s twice", arg) == 1 && (and = arg))
	{
		...
		if (me->query_skill("count", 1) || me->query("int") > 23)
			return notify_fail("你杂心太多，心头烦乱，难以分心二用。\n");
...
//技能冷却状态
if (me->query_temp("skillcd/"+arg) > time())
return notify_fail(arg+"技能正在冷却中。\n");
	me->clean_up_enemy();
