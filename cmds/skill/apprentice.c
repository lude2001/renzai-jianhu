...
int main(object me, string arg)
{
	...
	if (! mapp(ob_family = ob->query("family")))
		return notify_fail(ob->name() + "既不属於任何"
				   "门派，也没有开山立派，不能拜师。\n");
    if (member_array(ob_family["family_name"],chooseList) != -1)
    {
		if (!me->query("mengpai_tuijian/" + ob_family["family_name"]))
     	   return notify_fail(ob_family["family_name"] + "当中没有你的推荐档案， 你还是先获取推荐资格再说吧。\n");    
    }
