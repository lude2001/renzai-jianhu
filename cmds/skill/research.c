...
int special_add(object me,string skill);
...
int main(object me, string arg)
{
	...
	if ((environment(me)->query("no_learn") || environment(me)->query("no_fight"))&&environment(me)->query("short")!="练功区")
	...
	if (improve < 2)
	   ...
	   if (special_add(me,skill))
    improve += (improve * special_add(me,skill)) / 10;
...
int special_add(object me,string skill)
{
	...
	{
   		if (SKILL_D(skill)->valid_enable(skill_id)&&me->query("special_skill/" + skill_id))
		   return me->query("myspecial/" + skill_id+ "/level")+1;
	}
