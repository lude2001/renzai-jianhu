...
void auto_learn(object me, string teacher,string skill,int times)
{
	...
	if(!me||!me->query_temp("pending/auto_learn")||!present(teacher,environment(me)))
		...
		return;
    if (me->query_temp("learnjb"))
    {
    remove_call_out(me->query_temp("learnjb"));//按照句柄清除
    me->delete_temp("learnjb");
    }
    
	if(me->is_busy())
	jb = call_out("auto_learn",1,me,teacher,skill,times);
	...
	else if(me->query("neili")<302 && me->query("max_neili")>400)
	{
	 ...
	 jb = call_out("auto_learn",10,me,teacher,skill,times);
	}
...
int main(object me, string arg)
{
	...
	if (times < 1 || times > 99999)
		return notify_fail("学习次数最少一次，最多也不能超过十万次。\n");
	...
	//主线任务第一次学习技能
	if (me->query("maintask")==1)
	{
	me->set("study_skill",1);
	"/cmds/usr/maintask"->task_zt(me);
	}
