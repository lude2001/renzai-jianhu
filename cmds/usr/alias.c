...
int main(object me, string arg)
{
	int i;
	mapping alias;
	string verb, replace, *vrbs, *cmds, al, cmds_te, temp;
	...
	{
		alias = me->query_all_alias();
		sscanf(arg, "%s %s", al, cmds_te);
		cmds = explode(cmds_te, "#");
		if (sizeof(cmds) && sizeof(alias))
		{
			if (sizeof(cmds) > 5)
			{
				write("alias不能设定超过五条指令。\n");
				return 0;
			}
			
			/*foreach(temp in cmds)
			{
			    if (temp == al)
			    {
			        write("无法包含要设置的名字。\n");
				...
				if (sizeof(filter_array(keys(alias), (: $1 == $2 :), temp)))
				{
					write("无法包已含设定的alias。\n");
					return 0;
				}
