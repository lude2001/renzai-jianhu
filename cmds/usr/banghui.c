	...
	if( !arg )
	{
		...
		else
		{
			...
			if(me->query("banghui/zhiwei")=="帮主")
			{
				...
				text += ZJSEP"建设秘技阁:banghui mijige";
			...
			if (playerp(me)) 
			{
			text += ZJSEP"捐赠黄金:banghui gx";
			text += ZJSEP"捐赠师门贡献:banghui smgx";
...
/*		 if (!BANGHUI_D->query_banghui(short+"/gongxian/"+me->query("id"))&&BANGHUI_D->query_banghui(short+"/gongxian/"+me->query("name")))
	...
	{	
		if(!(short=me->query("banghui/name")))
		return notify_fail(ZJOBLONG"你没有帮会怎么建设秘技阁！\n");

		if (me->query("yuanbao") < 1000)
		return notify_fail("建设秘技阁需要1000元宝。\n");

		if(BANGHUI_D->query_banghui(short+"/mijige"))
		return notify_fail(ZJOBLONG"帮会已经建设秘技阁了!\n");
	...
	else if( sscanf(arg,"smgx %d",page)== 1 )
	{
		...
		if (page < 10)
			return notify_fail(ZJOBLONG"最少10师门贡献起捐！\n");
		if (page > 10000)
			return notify_fail(ZJOBLONG"最多10000师门贡献！\n");
		
		if (me->query("gongxian") < page)
			return notify_fail(ZJOBLONG"师门贡献不够"+page+"！\n");
    ...
    else if (arg == "smgx")
	{
		...
		return notify_fail(INPUTTXT("请输入要捐献的师门数量("HIY"1：1兑换"NOR")：","banghui smgx $txt#")+"\n");
	}
