...
int main(object me, string arg)
{
  ...
  msg += sprintf("┃  %-5s %-22s%-10s %5d   "NOR"\n"NOR,chinese_number(i+1),list[i]->query("name")+"("+
        capitalize("*****")+")", list[i]->query("family")?  list[i]->query("family/family_name"):"普通百姓",
        get_score(list[i]));
  ...
  msg += sprintf("┃  %-5s %-22s%-10s %5d   "NOR"\n"NOR,chinese_number(i+1),me->query("name")+"("+
        capitalize("*****")+")", me->query("family")?  me->query("family/family_name"):"普通百姓",
        get_score(me));
                }
                         }
        msg += HIC+"┗━━━━━━━━━━━━━━━━━━━━━━┛\n";
...
int get_score(object ob)
{
    ...
    //    score += ob->query("max_neili")/10;
        ...
        score += (int)ob->query_skill("force")+(int)ob->query_skill("dodge")+(int)ob->query_skill("parry");
