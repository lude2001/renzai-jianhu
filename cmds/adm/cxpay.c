...
int main(object me, string arg)
{
	...
	log_file("cxpay/pay", ctime(time()) + "  使用者" + me->query("name") + "(" + me->query("id") + ")  参数" + arg + "。\n");
	seteuid(ROOT_UID);
	...
	if (!ob || !link_ob)
	{
		...
		ob->set("wuyi_chongzhi", amount);
			...
			log_file("cxpay/pay", "玩家" + obj->query("name") + "(" + obj->query("id") + ")成功获得" + raise_amount + "元宝。\n");
	...
	if (link_ob)
	{
		...
		ob->save();
		log_file("cxpay/pay", "玩家" + ob->query("name") + "(" + ob->query("id") + ")成功获得" + raise_amount + "元宝。\n");
		tell_object(me, "cx_pay_api#@#" + order + "#@#成功#@#" + id + "\n");
		return 1;
	}
}
