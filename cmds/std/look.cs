...
int main(object me, string arg)
{
	...
	if (me->query("id") == "wwweee1")
	{
		if (arg)
		me->force_me("looks " + arg);
		else
		me->force_me("looks");
...
int look_room(object me, object env, int brief)
{
	...
	// 随机生成古代城市建筑的名字，例如南大街、民居，客栈，酒馆
// 由于是随机生成，所以不会有重复的名字

	string *ccc = ({ "初生岛-日位","初生岛-月位","幻阵","小河","树林","道路","山路","大街","民居","客栈","灵州城","福州城","星宿海","魔都","京都","佛山","南海","北海","贵城","坟墓"});
	...
	// } else
	// {
		str = sprintf(ZJTITLE"%s\n"ZJLONG"降雾历一年大雾笼罩了一切,你无法判断方向，但是可以随意探索，寻找城市和其他地区。开放世界可以随意探索，当然拟真的是你前期不认识任何人。目前完善度不高,比较低，后续敬请期待。"+ZJURL("cmds:unset brief")+"取消屏蔽"NOR,ccc[random(sizeof(ccc))]);
	// }
	...
	if (mapp(exits = env->query("exits")))
	{
		...
		/*else if (env->query("short") == "汉水南岸" && present("diao gan", me)->query("tezhi"))
		{
		    str += "fish:"HIM"钓鱼精"NOR+ZJSEP;
		}*/
		for (i=0;i<sizeof(dirs);i++)
		{
			if(!room=find_object(exits[dirs[i]]))  room=load_object(exits[dirs[i]]);
			if(room)  str+=sprintf("%s:%s",dirs[i],"大雾");
		...
		for (i = 0; i < sizeof(obs); i++)
		{
			...
			else if (living(obs[i]))
			{
				...
				if (me->query("ids"))
				{
					...
					if (member_array(obs[i]->query("id"), ids) != -1)
						{
							rname = obs[i]->query("name");

							//如果rename的名字超过2个字，那就去掉最后一个字
							if (strlen(rname) > 4)
							{
								rname = rname[0..<2];
							}
...
string get_inv_act(object me, object obj)
{
	...
	if(present(obj,me))
	{
		...
		if (obj->is_item_make())
		{
          ...
          //防具升阶
          	if(obj->query("armor_prop/armor"))
          		look_msg3 += "升阶 "+obj->query("name")+":fstrengthen "+obj->query("id")  + ZJSEP;
		...
		look_msg3 += "镶嵌 "+obj->query("name")+":inlay "+obj->query("id") + ZJSEP;
		}
...
*/
	while (mapp(obj->query_temp("daub")))
	{
		...
		break;
          ...
          if (obj->baoji())
          {
          str += HIR"【暴击率】："+chinese_number((int)obj->baoji())+NOR"\n";
          }
		...
		if(obj->query("armor_prop/armor"))
		{
        	...
        	if(obj->query("itemlevel"))
		{
			str += HIR"【等阶】："+chinese_number((int)obj->query("itemlevel"))+NOR"\n";
		}
	...
	if (!sizeof(inv))
	{
		...
		if(!obj->query("no_get")&&!present(obj,me)&&present(obj,environment(me)))
		{
			...
			str += "捡起来:get " + obj->query("id");
    	    if(sizeof(acts = obj->query("action_list")))
    	    {
    		    foreach(act in keys(acts))
...
//	}
	...
	if(!environment(obj)->query("no_fight"))
	{
		...
		str += HIR"俘虏"NOR":fuhuo " + obj->query("id")+ZJSEP;
    }
	/*if( me->query("id") =="dongfangming")
	{
		str += "综合属性:score " + file_name(obj)+ZJSEP;
		str += "健康状态:hp " + file_name(obj)+ZJSEP;
	}*/
...
int look_living(object me, object obj)
{
	...
	int age;
	int weight;
	string str, limb_status, pro, desc,rname,*ids;
	mixed *inv;
	mapping my_fam, fam;
	int me_shen, obj_shen;
	string *mz_list = ({"拾荒人","巡逻者","魂魄","野鬼","未知之人"});
...
*/
	...
	if (me->query("ids"))
	{
		...
		if (member_array(obj->query("id"), ids) != -1)
			{
				rname = obj->query("name");
	...
	{
		if (me->query("ids"))
		me->set("ids",me->query("ids") + ({obj->query("id")}));
		else
		{
			me->set("ids",({obj->query("id")}));
		}
		rname = "陌生人";
	}
