...
int look_room(object me, object env, int brief)
{
	...
	if (mapp(exits = env->query("exits")))
	{
		...
		for (i=0;i<sizeof(dirs);i++)
		{
			...
			if(room)
			{
			    if (!room->query("owner_build") || (room->query("owner_build") && room->query("owner_build") == me->query("id")) || (room->query("owner_build") && me->query("env/auto_home")) )
...
string desc_of_objects(object *obs,object me)
{
	...
	if (obs && sizeof(obs) > 0)
	{
		...
		for (i = 0; i < sizeof(obs); i++)
		{
			...
			if(playerp(obs[i]))
			{
			    string roomName = environment(obs[i])->query("short");
			    
			    
			    if (roomName == "武庙" || roomName == "宝昌客栈" || roomName == "打铁铺")
			    continue;
			    
			    if (obs[i]->full_chongmai())
			    str += HIM+rname+NOR":look "+file_name(obs[i]);
			    else
	...
	if(present(obj,me))
	{
		...
		if (obj->is_item_make())
		{
          ...
          //防具升阶
          	...
          	{
          	    look_msg3 += "祭炼 "+obj->query("name")+":item_tz "+obj->query("id")  + ZJSEP;
...
*/
	...
	//str = ZJOBLONG+(!userp(obj)?obj->short():obj->name())+"\n";
	str = ZJOBLONG+ obj->short() + "\n";
	str += "气血:" + obj->query("qi") + "/" + obj->query("eff_qi") + "\n";
