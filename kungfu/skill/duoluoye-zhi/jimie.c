...
//	object weapon;
	...
	ap = me->query_skill("finger", 1) * 2 + me->query_skill("duoluoye-zhi", 1);
	...
	damage = ap + random(ap / 4);
	 ...
	 if (playerp(target))
	 {
	  ...
	  if (objectp(weapon = me->query_temp("armor/hands")) && weapon->query("magic/skill/duoluoye-zhi"))//拥有胡家专属宝石的加成
         damage *= 3;
