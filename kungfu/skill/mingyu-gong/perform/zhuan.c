...
int perform(object me, object target, int amount)
{
        ...
        message_combatd("$N肌肉里的每一恨筋络，每一恨骨头都彷佛能看得清清楚楚，说不出的诡秘可怕！"NOR"\n",me);
...
void give_effect(object me,int extra,int level)
{
        ...
        message_combatd(HIW"$N内力非但不向外挥发，反而向内收缩，以丹田为中心形成真气漩涡！"NOR"\n",me);
...
void take_effect(object me,object target,int level)
{
    ...
    switch (i)
	{
        case 0:
                if (level>=10)
                                msg = GRN"$N身处明玉功的真气漩涡中，突觉举步唯艰！。"NOR"\n";
                else
                                msg = YEL"$N身处明玉功的真气漩涡中，突觉举步唯艰！。"NOR"\n";
                message_combatd(msg,target);
                if (level>=10)
        ...
        case 1:
                ...
                if (!target->query("neili") && level<11) {
                                msg = YEL"$N内力耗尽，脚步蹒跚，摇摇欲坠！"NOR"\n";
                                message_combatd(msg,target);
                                if (target->query_busy()<2) target->start_busy(2);
                                break;
                }
                if (level>=11)
                                msg = RED"$N身处明玉功的真气漩涡i中，全身气血如融雪般开始消失！"NOR"\n";
                else
                                msg = WHT"$N身处明玉功的真气漩涡中，全身功力如融雪般开始消失！"NOR"\n";
                message_combatd(msg,target);
                max_force = target->query("max_neili");
                if(level>=11 && max_force<=1200)    max_force = 1200;
                target->add("neili",-max_force/15);
