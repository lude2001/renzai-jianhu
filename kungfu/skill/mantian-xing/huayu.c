...
int perform(object me, object target)
{
	...
	if (playerp(me) && ! me->query("can_perform/mantian-xing/xing"))
		return notify_fail("你还不会使用「" HIR "漫天花雨" NOR "」这一绝技。\n");

	if (me->query_skill("hanxing-bada", 1) < 180)
		return notify_fail("你的寒星八打修为不够，无法使用满天花雨。\n");
	if (! target) target = offensive_target(me);

	if (! target || ! me->is_fighting(target))
		return notify_fail("「" HIR "满天花雨" NOR "」只能在战斗中对对手使用。\n");
	...
	if (weapon->query_amount() < 15)
		return notify_fail("你手中至少要有十五枚暗器才能施展「" HIR "满天花雨" NOR "」这一招。\n");

	if ((skill = me->query_skill("mantian-xing", 1)) < 80)
		return notify_fail("你的满天星不够娴熟，不会使用「" HIR "满天花雨" NOR "」这一招。\n");

	if ((int)me->query_skill("force") < 150)
		return notify_fail("你的内功火候不够，不能使用「" HIR "满天花雨" NOR "」这一招。\n");
	...
	if (my_exp / 2 + random(my_exp * 3 / 2) > ob_exp)
	{
		if (target->query_skill("parry") < me->query_skill("throwing"))
		{
			...
			target->add_temp("apply/attack", -120);
