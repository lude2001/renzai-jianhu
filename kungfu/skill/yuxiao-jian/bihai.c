...
int perform(object me, object target)
{
	...
	ap = me->query_skill("sword", 1) / 2 +
	     me->query_skill("force") + me->query_skill("chuixiao-jifa") + me->query_skill("martial-cognize", 1) / 2 + me->query_skill("sword-cognize", 1) / 2;
	dp = target->query_skill("force") * 2;

	lvl = (int) me->query_skill("yuxiao-jian", 1) / 5 + 10;
	if (lvl > 40) lvl = (lvl - 40) / 3 + 40;

	jing = (int) target->query("max_jing");
	if (jing>10000) jing = (int) me->query("max_jing");
    
    if (target->query("max_jing") > me->query("max_jing") * 3 / 2)
    jing /= 5;
