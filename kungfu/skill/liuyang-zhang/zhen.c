...
int perform(object me, object target)
{
        ...
        if (me->query("family/family_name") != "灵鹫宫" && me->query("family/family_name") != "逍遥派")
                return notify_fail("你的非逍遥派或者灵鹫宫弟子，无法使用「一掌震天」之力。\n");
                        
        if (me->query_skill("bahuang-gong", 1) < 240 && me->query("family/family_name") == "灵鹫宫")
                return notify_fail("你的八荒六合惟我独尊功修为不足，无法使用「一掌震天」之力。\n");
                
        if (me->query_skill("beiming-shengong") < 240 && me->query("family/family_name") == "逍遥派")
                    return notify_fail("你的八荒六合惟我独尊功修为不足，无法使用「一掌震天」之力。\n");
