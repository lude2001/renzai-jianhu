...
int exert(object me, object target)
{
	...
	if (me->query_temp("armor/hands"))
	    hands = me->query_temp("armor/hands");
	
	if ((objectp(hands) && hands->query("magic/skill/hamagong") && !me->query_temp("weapon"))?(random(10)):(ap / 3 + random(ap) > dp))
	{
		me->add("neili", -400);
		if (objectp(hands) && hands->query("magic/skill/hamagong") && !me->query_temp("weapon"))
		    skill_cd(me, "tui", 2);
		else
		    me->start_busy(2);
		damage = (ap - dp) / 10 + random(ap / 10);
		
		if (objectp(hands) && hands->query("magic/skill/hamagong"))
		    damage *= 5;
