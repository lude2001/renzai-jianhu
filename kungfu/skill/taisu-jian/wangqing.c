...
int perform(object me, object target)
{
	...
	if (me->query_skill("taisu-jian", 1) < 500)
		return notify_fail("你的太素剑诀修为不足，还不会使用太上忘情！\n");
	if (me->query_skill("taisu-dodge", 1) < 500)
		return notify_fail("你的太素玲珑步修为不足，还不会使用太上忘情！\n");
	if (me->query_skill("taisu-force", 1) < 500)
		return notify_fail("你的太素心经修为不足，还不会使用太上忘情！\n");
	...
	message_vision(HBWHT "$N贯通古墓武学，使出了太上忘情的绝学之精髓！\n" NOR, me, target);
	...
	if (userp(me) && me->query("env/no_combat_msg") < 4)
		tell_object(me, HIW "该绝招对敌人气血的外伤为" + damage + "，内伤为" + damage / 2 + "。\n" NOR);
	...
	if (me->query_skill("taisu-jian") > 2000 && me->query_skill("taisu-moon") > 2000)
	{
		...
		if (random(my_power) > op_power / 3)
		{
			...
			else
			{
				...
				me->add("jingli", -200);
				if (userp(me) && me->query("env/no_combat_msg") < 4)
					tell_object(me, HIW "该绝招对敌人气血的外伤为" + damage / 3 + "，内伤为" + damage / 5 + "。\n" NOR);
			}
		}
		else
		{
