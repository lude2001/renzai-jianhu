...
int perform(object me, object target)
{
	...
	if (random(me->query_skill("blade") + me->query_skill("martial-cognize", 1) / 2) > (int)target->query_skill("force") / 2)
	{
		damage = me->query_skill("blade");
		damage += me->query_skill("martial-cognize", 1) / 2;
		damage = damage / 2 + random(damage / 2);
		
		  if (weapon->query("magic/skill/xue-dao"))	 
          {
	    	me->receive_curing("qi",damage / 3);
	    	damage += 1000;
          }
          else
          {
             me->receive_curing("qi",damage / 5);
          }
