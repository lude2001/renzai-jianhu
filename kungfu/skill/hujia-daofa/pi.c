...
int perform(object me, object target)
{
	...
	ap = me->query_skill("blade") + me->query_skill("force") + me->query_skill("martial-cognize") /  2;
	dp = target->query_skill("force");
	
	if (weapon->query("magic/skill/hujia-daofa"))//拥有胡家专属宝石的加成
	ap += me->query_skill("huajia-daofa");//增加胡家刀法等级
	
	if (ap / 2 + random(ap / 2) > dp)
	{
		damage = ap * 2 + random(ap);
		damage += me->query("jiali");
		damage += me->query_skill("lengyue-shengong");
		if (weapon->query("magic/skill/hujia-daofa"))//拥有胡家专属宝石的加成
		{
		damage += 1000;
		tell_object(me,"只见一声脆响，你感觉"+weapon->name()+"上的蓝色翡翠爆发出强烈的光芒，顿时对开天辟地的感悟加深\n");
		}
		me->add("neili", -(damage / 2));

            	if (weapon->query("magic/skill/hujia-daofa-daohun"))//拥有胡家专属宝石的加成
		...
		{       
			...
			if (target->query_temp("lengyue-shengong_freeze"))
			{
				msg += COMBAT_D->do_damage(me, target, WEAPON_ATTACK, damage, 33,
							   ...
							   "一口鲜血！"NOR"\n");
				if (weapon->query("magic/skill/hujia-daofa-daohun"))//拥有胡家专属宝石的加成
            	target->delete_temp("lengyue-shengong_freeze");	 
			}
		} else
		{
