...
int perform(object me, object target)
{
	...
	{
		if (target->query_temp("lengyue-shengong_freeze") || target->query_condition("shenzhao"))
		{
			target->add_temp("apply/parry", -lvl / 2);
			target->add_temp("apply/dodge", -lvl / 2);
			target->set_temp("xuanming_wuji", 1);
		}
		...
		if (target->query_temp("xuanming_wuji"))
		{
			target->add_temp("apply/parry", lvl / 2);
			target->add_temp("apply/dodge", lvl / 2);
			target->delete_temp("xuanming_wuji");
		}
	} else
	{
