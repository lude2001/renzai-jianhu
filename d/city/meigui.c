...
LONG );
	...
	set("no_sleep_room",1);
	set("item_desc", ([
	    "菜单": "承办各式酒席, 欢迎惠顾：\n" + ZJOBACTS2 + "祝寿宴	(2两黄金):order birthday" + ZJSEP + "订婚宴	(3两黄金):order marry" + ZJSEP + "群豪宴	(5两黄金):order player" + ZJSEP + "结  束:order end\n",
	]));
...
int do_look(string arg)
{
    ...
    if (objectp(find_object(arg)))
    {
        ...
        {
            arg->set("action_list", (["吃" : "eat " + arg]));
        }
        if (arg->query("liquid/remaining"))
        {
            arg->set("action_list", (["喝" : "drink " + arg]));
        }
    }
...
int do_broadcast(string arg)
{
	...
	message("channel:chat", RED+"【江湖通告】"+mesg+NOR + "\n", users() );
	message("channel:chat", GRN+"【江湖通告】"+mesg+NOR + "\n", users() );
	message("channel:chat", CYN+"【江湖通告】"+mesg+NOR + "\n", users() );
	return 1;
}
