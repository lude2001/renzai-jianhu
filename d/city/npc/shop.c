...
int main(object me, string arg)
{
	...
	if (! arg)
	{
		...
		for(i=0;i<sizeof(all_file);i++)
		{
			...
			price = yuan_bao + "通宝";
			str += sprintf("%s"ZJBR"%s:heixin_shop look %s"ZJSEP,file->query("name"),price,file);
		}
		write(str+"\n");
		return 1;
	}
	else if (sscanf(arg,"look %s",file)==1)
	{
		...
		str += "购买1个:heixin_shop buy 1 "+file+ZJSEP"批量购买:heixin_shop buy "+file+"\n";
		write(str+"\n");
		return 1;
	}
	else if (sscanf(arg,"buy %s",file)==1)
	{
		...
		{
			write(INPUTTXT("你想买多少个"+file->query("name")+"？"ZJBR"请确认你的背包空间足够。","heixin_shop buy $txt# "+file)+"\n");
			return 1;
		}
