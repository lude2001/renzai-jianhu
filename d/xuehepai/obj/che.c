...
int do_ride(string arg)
{
        ...
        if (fam && fam["family_name"]=="合欢宗" && present("ren kuang",environment(me)))
                return notify_fail("任狂还没答应让你开呢。\n");
       if (fam && fam["family_name"]=="合欢宗" && ((int)me->query("combat_exp")<500000))
        ...
        if(!fam || fam["family_name"]!="合欢宗") {
                ...
                if (!(me->is_fighting()))
                {
                message_vision("$N见周围没有了合欢宗弟子，放心大胆的跃上血河车的车厢！\n",me);
                me->set_temp("xueheche",1);
                return 1;
                }
                return notify_fail("你先把合欢宗弟子解决了去驾车也不迟。\n");
        }
        }
...
int do_enter(string arg)
{
        ...
        if(!fam || fam["family_name"]!="合欢宗")
        {
                ...
                if (!(me->is_fighting()))
                {
                message_vision("$N见周围没有了合欢宗弟子，放心大胆的进入血河车！\n",me);
                me->move("/d/xuehepai/chenei");
                }
                return notify_fail("你先把合欢宗弟子解决了再进去也不迟。\n");
        }
