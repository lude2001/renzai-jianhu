...
int recognize_apprentice(object ob, string skill)
{
    if (skill != "fufa")
    {
        command("say 我这里只传授符法基础。\n");
        return -1;
    }
    
    if (ob->query("family/family_name") != "武当派" && ob->query("family/family_name") != "全真教")
    {
        command("say 非我道教中人无法学习符法基础。\n");
        return -1;
    }
    
    if (ob->query_skill(skill, 1) >= 300)
    {
        command("say 最多只能学到300级。");
        return -1;
    }
