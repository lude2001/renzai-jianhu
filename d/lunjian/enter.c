...
int jjsign()
{
	...
	if(localtime(now)[6]!=LUNJIAN_DAY||localtime(now)[2]!=LUNJIAN_HOUR||localtime(now)[1]>LUNJIAN_PRE)
		return notify_fail(ZJOBLONG"报名时间："+LUNJIAN_HOUR+":00-"+LUNJIAN_HOUR+":31。\n");
	...
	tell_object(me,"报名成功，请在此稍后，"+LUNJIAN_HOUR+"点30分自动进入竞技场中。\n");
	set_heart_beat(1);
	return 1;
}
...
private void auto_check()
{
	...
	if(!sizeof(rooms))
	{
	    last_fight_record += "--------第" + (turn + 1) + "轮--------"ZJBR;
			...
			else
			{
				...
				if (sizeof(total) > 5)
				{
					for(int s = 5;s < sizeof(total);s++)
					{
						...
						title_name = "第" + chinese_number(s-4) + "惊霄";
		...
		if(sizeof(us))
		{
			...
			log_file("lunjian", ctime(time()) + ob->query("name") + "轮空晋级，积分增加1。\n");
		}
		turn++;
		return;
	}
	...
	{
	    rooms[i]->add("fight_time",2);//每次心跳增加两秒钟
		inv = filter_array(all_inventory(rooms[i]),(: playerp($1) :));
		if(!sizeof(inv))
		{
			last_fight_record += rooms[i]->query("record")+ ZJBR"战斗用时:"+ rooms[i]->query("fight_time") + "秒" + ZJBR;
			destruct(rooms[i]);
		}
		else if(sizeof(inv)==1)
		{
			...
			rooms[i]->set("record",rooms[i]->query("record") + sprintf("(%s胜出)",inv[0]->query("name")));
			log_file("lunjian", ctime(time()) + inv[0]->query("name") + "胜利晋级，积分增加1。\n");
			check_win(inv[0]);
		}
		else if (rooms[i]->query("fight_time") >= 120 && sizeof(inv) == 2) //战斗两分钟判断平手
		{
		    rooms[i]->set("record",rooms[i]->query("record") + sprintf("(平手)"));
			...
			log_file("lunjian", ctime(time()) + inv[1]->query("name") + "胜利晋级，积分增加1。\n");
			check_win(inv[1]);
		}
	}
}
...
private void init_player(object me)
{
	...
	me->set_override("net_dead", (: call_other, __FILE__, "check_out" :));//断线判断失败
...
int check_out(object me)
{
	...
	tell_object(me, HIR "\n你觉得眼前一阵模糊...这下完了！，你在本轮论剑中晋级失败,获得1000经验，1000潜能！"NOR"\n");
...
int check_win(object me)
{
    ...
    me->add("yuan_bao",5);
	tell_object(me, HIR "\n你在本轮论剑中获胜晋级,获得2000经验，2000潜能，5个通宝！"NOR"\n");
