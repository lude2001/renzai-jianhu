	...
	else
	{
    	for(i = 0; i < sizeof(team); i++)
    	{
    		...
    		if (fuben_jiaofei < 50)
    		{
        		if (i == 0)
        		{
        			team[i]->add("combat_exp", 250);
        			team[i]->add("potential", 350);
        			tell_object(team[i], "你获得了二百五十点经验和三百五十点潜能。\n");
        		}
        		else
        		{
        			team[i]->add("combat_exp", 200);
        			team[i]->add("potential", 300);
        			tell_object(team[i], "你获得了二百点经验和三百点潜能。\n");
        		}
        	}
        	else
        		tell_object(team[i], "你今日副本完成超过五十次，无法再获得经验潜能。\n");
    	}
    }
