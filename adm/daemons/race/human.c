...
void setup_human(object ob)
{
	...
	if (userp(ob) || undefinedp(my["max_qi"]))
	{
		...
		{
			my["max_qi"] += (x-180)*10 + (x + 150 ) * (y + 100) / 100;
		}
		...
		if (ob->query("special_skill/ghost"))
			...
			if (ob->query("titlea/max_qi"))
			my["max_qi"] += my["titlea"]["max_qi"];
		if (xflv > 180)
		{
		...
		if (ob->query("miji/wu"))
		my["max_qi"] += (ob->query("miji/wu")) * 100;
		
		if (ob->query("yijing/other_add/qi"))
		my["max_qi"] += ob->query("yijing/other_add/qi");
	}
