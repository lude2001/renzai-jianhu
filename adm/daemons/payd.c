	...
	if(!VIP_D->query_vips("order")||(member_array(datas["order_id"],VIP_D->query_vips("order"))==-1))
	{
		...
		if(!(obj=find_player(datas["user_id"])))
		{
			...
			if(obj->restore())
			{
				...
				obj->add("chonzhi/rmb",rmb);//独立的充值数值
...
//int reward_yuanbao;
   ...
   {  
     ...
     reward_obj = new("/clone/vip/xumibei");//须弥碑             
       if (objectp(reward_obj))
      {
      reward_obj->move(ob);
      tell_object(ob,"由于您对本游戏的杰出贡献，特此奖励您"+reward_obj->query("name")+"哦，请注意查收。\n");
      }
   }
   ...
   reward_obj = new("/clone/vip/butian");//补天石
     if (objectp(reward_obj))
      {
      reward_obj->move(ob);
      tell_object(ob,"由于您对本游戏的杰出贡献，特此奖励您"+reward_obj->query("name")+"哦，请注意查收。\n");
