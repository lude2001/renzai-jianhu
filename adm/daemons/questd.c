...
int check_shimen(string arg,object me);
int check_time(object me);
// 发放任务者列表
mapping mlist = ([
    "合欢宗" : "/d/xuehepai/npc/gui1.c",
...
public void skill_bonus(object me, object who)
{
	string *sk_list = ({
		"qishang-quan",
		"qishang-quan",
...
public void special_bonus(object me, object who, mixed arg)
{
	string *ob_list = ({
        "/clone/baoshi/cagate",
		"/clone/baoshi/cjade",
		"/clone/baoshi/cdiamond",
		"/clone/baoshi/ccrystal",
...
//在原来2*percent的基础上再提高60% by ming on 2016.10.26 by mao 再加20%
        percent =percent*15/5;
        
    //注册时间小于7天奖励翻倍
    if ((time() - who->query("birthday")) / 86400 < 7)
        percent *= 2;
...
//     percent = percent*(140+who->query("relife/zhuanshi")*10)/100;
	...
	who->add("gongxian",1);
	who->add("shimen/lun",1);//用于验证
		//主线任务第一次完成师门任务
	if (who->query("maintask")==3)
	{
	"/cmds/usr/maintask"->task_zt(who);
	}
...
int ask_quest(object me, object who)
{
	...
	if (exp < 30000)
	{
		...
		ob->set_name("red" + ob->query("name"), ob->query("id") + " shimen");
...
/*		if (who->query("score") < 2000)
	...
	*/
		...
		if (who->query("shimen/today")>=(1500 * (1 +  (time()-1652168569) /  604800)))
		{
			message_sort("$N对$n摇了摇头：“这位" + RANK_D->query_respect(who) +"，注意身体呀，等下周再来领取新的任务吧！”"NOR"\n", me, who);
			return 1;
		}

  	if (who->query("shimen/lun")>=15)
  	{
    if (check_shimen("yanzhen",who))
  	return 1;
  	}
...
int remote_bonus(object me)
{
	...
	if ((quest_count % 50) == 0)
	{
		  ...
		  onecard->move(me);
		msg += "真是不赖，居然连着" + chinese_number(quest_count) + "次都没有失手,奖励你一张单抽卡。";
	}
...
int start_quest(object me, mapping quest)
{
	...
	if (me->query("shimen/today")>=(1500 * (1 +  (time()-1652168569) /  604800)))// 从开服开始算
	{
				   ...
				   mapping new_quest)
{
    ...
    quest_count_ch = who->query("quest_count") % 50;
    
    if (quest_count_ch < 45)
    {
        who->delete("quest_count_check");
    }
    else
    {
    	if (quest_count_ch >=45 && quest_count_ch <50)
    	{
    	    if (random(2) && !who->query("quest_count_check"))
    	    {
    	        who->set("quest_count_check", 1);
    	        tell_object(who, "本次师门任务需要上交首级，请割下头颅回师门上交。\n");
    	        return 0;
    	    }
    	}
    	else if (quest_count_ch == 0 && !who->query("quest_count_check"))
    	{
    	    tell_object(who, "本次师门需要上交头颅，请割下头颅回师门上交。\n");
    	    return 0;
    	}
	}
...
{
//暂时关闭
me->delete("shimen/lun");
return 1;
if (!objectp(me))
tell_object(find_player("baiyi666"),"师门验证系统出错\n");
 if (arg=="yanzhen")
 {
 int one,two,all;
 string sw;
 ...
 two = random(100);
 all = one+two;
 sw = one + " + " + two + " = "+chinese_number(all) ;
 remove_call_out("check_time");
 me->set_temp("check_shimen",chinese_number(all));
 call_out("check_time",20,me);
 tell_object(me,INPUTTXT(sw+"请您验证，请输入中文结果","$txt#")+"\n");
 input_to("check_shimen",me);
  return 1;
}
else if (strsrch(arg,me->query_temp("check_shimen"))!=-1)
{
me->delete("shimen/lun");
tell_object(me,"验证成功,祝您游戏愉快，并为您恢复状态哦。\n");
"/cmds/imm/recover"->main(me);//恢复
 return me->delete_temp("check_shimen");
 }
 else //验证失败，重新验证
 return check_shimen("yanzhen",me);
}