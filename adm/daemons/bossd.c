...
//刀剑副本开始
int start_blade_sword_fuben(int state)
{
	object huyidao,miaorenfeng;

	message("story",HIY"刀剑归墟已经开启，请诸位速速前往"NOR"。\n", users());

	huyidao = new("/d/fuben/newboss/npc/hu");
	miaorenfeng = new("/d/fuben/newboss/npc/miao");

	huyidao->move("/d/fuben/newboss/map/guixu");
	miaorenfeng->move("/d/fuben/newboss/map/guixu");

	huyidao->kill_ob(miaorenfeng);
	startDjfuben = state;
...
//查询刀剑副本的状态
int query_start_blade_sword_fuben()
{
	return startDjfuben;
}
...
void heart_beat()
{
	...
	//每天下午6点到晚上11点开启副本,并且是整点
	if (r[1]==1&&(r[2] >= 18 && r[2] <= 23))
	{
	    if (!startDjfuben)
		start_blade_sword_fuben(1);
	}
	else if (r[1] >= 30)
	{
	startDjfuben = 0;
