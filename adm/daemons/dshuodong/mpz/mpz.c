...
int start()
{
    ...
    log_file("menpairuqin", ctime(time()) + "门派入侵选定" + load_object(zhangmen)->query("family/family_name") + "敌对为：" + enemy + "。1234\n");
	CHANNEL_D->do_channel(load_object(zhangmen), "chat", enemy + "的弟子真是太嚣张了，已经打上山门了！\n");
	log_file("menpairuqin", ctime(time()) + "门派入侵选定" + load_object(zhangmen)->query("family/family_name") + "敌对为：" + enemy + "。\n");
