...
mapping xuewei = ([
	...
	"左阳经" : ({ "会阴", "阴交", "气冲", "横骨", "大赫", "气", "四满", "中注", "肓俞", "商曲", "石关", "阴都", "通谷", "幽门", "关门", "太乙", "滑肉门", "天枢", "外陵", "大巨", "水道", "归来", "水突", "气舍", }),
	"左阴经" : ({ "会阴", "阴交", "气冲", "横骨", "大赫", "气", "四满", "中注", "肓俞", "商曲", "石关", "阴都", "通谷", "幽门", "关门", "太乙", "滑肉门", "天枢", "外陵", "大巨", "水道", "归来", "水突", "气舍", }),
	...
	"足三阳经":({ "迎香","禾髎","扶突","天鼎","巨骨","手五里","阳溪","商阳","丝竹空","角孙","天牖","肩髎","清冷渊","四渎","中渚","关冲","听宫","颧髎","天容","天窗","天宗","小海","后溪","少泽",}),
	"足三阴经":({ "天府","尺泽","孔最","列缺","经渠","太渊","鱼际","少商","天池","曲泽","郄门","间使","内关","大陵","劳宫","中冲","极泉","少海","灵道","通里","阴郄","神门","少府","少冲",}),
]);
...
varargs string do_score(object me,string arg)
{
	...
	{
		line += "真血减伤增加"+me->query("gain/jianshang")+"点。\n";
	}
	else if(name=="足三阴经")
	{
		line += "真血减伤增加"+me->query("gain/jianshang")+"点。\n";
...
int do_through(object me)
{
	...
	if(me->query("meridian/" + name) >= size)
	{
		...
		if (name == "左阳经") {//冲脉全部打通后一次奖励(120)
			num = 120;
			me->add("gain/damage", num );
		}else
		if (name == "左阴经") {//冲脉全部打通后一次奖励(120)
			num = 120;
			me->add("gain/damage", num );
		}else
		...
		if (name == "足三阳经") {//手三阴经全部打通后一次奖励(10)
			num = 10;
			me->add("gain/jianshang", num );
		}
		else
		if (name == "足三阴经") {//手三阴经全部打通后一次奖励(10)
			num = 10;
			me->add("gain/jianshang", num );
		}
	...
	else
	{
		...
		if (name == "左阳经") {//增加固定伤害力(24个穴道总共增加365伤害)
			if(index<=5)
			{
				num = 10;
				me->add("gain/damage", num);
		...
		if (name == "左阴经") {//增加固定伤害力(24个穴道总共增加365伤害)
			if(index<=5)
			{
				num = 10;
				me->add("gain/damage", num);
		...
		else if (name == "足三阳经") {//增加真血减伤(24个穴道总共增加66减伤)
			if(index<=5)
			{
				num = 1;
				me->add("gain/jianshang", num);
		...
		else if (name == "足三阴经") {//增加真血减伤(24个穴道总共增加66减伤)
			if(index<=5)
			{
				num = 1;
				me->add("gain/jianshang", num);
	...
	&& me->query("meridian/手三阳经") >=24 && name != "足三阴经" && name != "足三阳经" && name != "左阳经" && name != "左阴经")
	{
