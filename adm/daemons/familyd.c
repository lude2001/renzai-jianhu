...
//华山论剑，天下第一
//string lunjian_leader;
//天下第一奖励时间
//int lunjian_leader_bouns;
...
/*
public set_lunjian_leader(string id)
{
	lunjian_leader = id;
	save();
}

public string query_lunjian_leader()
{
	if(!lunjian_leader)
		return "";
...
public string bouns_family_leader(string fname, object me)
{
	...
	ob->move(me);
	log_file("bouns_family_leader",sprintf("%s：%s(%s)领取了%s首席奖励。",ctime(time()),me->query("name"),me->query("id"),fname));
	save();
	return msg;
}
...
public string bouns_lunjian_leader(object me)
{
	...
	msg = "";
	if (mapp(lunjian_leader))	s = keys(lunjian_leader);
...
public string bouns_lunjian_leader(object me)
{
	...
	if(!lunjian_leader||me->query("id")!=lunjian_leader)
		return "你是武林至尊么？\n";
	if(lunjian_leader_bouns&lunjian_leader_bouns/86400==time()/86400)
		return "武林至尊的奖励今日已经发放过了！\n";
	if(localtime(time())[6]==7&&localtime(time())[2]<6)
		return "竞技日0:00-6:00之间不能领取奖励！\n";

	lunjian_leader_bouns = time();
	me->add("balance",300000);
	msg = "你得到了30金币存款奖励。\n";
	me->add("potential",2000);
	msg += "你得到了2000点潜能奖励。\n";
	...
	msg += "你得到了"+ob->short()+"的奖励。\n";
	ob->move(me);
	log_file("bouns_lunjian_leader",sprintf("%s：%s(%s)领取了武林至尊奖励。",ctime(time()),me->query("name"),me->query("id")));
	save();
	return msg;
}*/
