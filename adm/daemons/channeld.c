...
#define TITLE_D "/adm/daemons/titled"
...
mapping channels = ([
	...
	"ultra": ([     "msg_speak": HIC "【宗师】%s：%s"NOR"\n",
			"msg_emote": HIC "【宗师】%s" NOR"\n",
			"msg_color": HIC,
			"name"     : "大宗师",
			"intermud" : GCHANNEL,
			"intermud_emote"    : 1,
	...
	"sos":  ([      "msg_speak": HIC "【求助】%s：%s"NOR"\n",
			"msg_color": HIC,
			"name_raw" : 1,
			"name"     : "求助",
		]),
	"party":([      "msg_speak": HIC "【帮派】%s：%s"NOR"\n",
			"msg_emote": HIC "【帮派】%s" NOR"\n",
			"msg_color": HIC,
			"only"     : "party",
			"name"     : "帮派",
			"omit_log" : 1,
		]),
	"family":([     "msg_speak": HIC "【同门】%s：%s"NOR"\n",
			"msg_emote": HIC "【同门】%s" NOR"\n",
			"msg_color": HIC,
			"only"     : "family",
			"name"     : "同门",
			"omit_log" : 1,
		]),
	"rultra":([     "msg_speak": HIC "【塞外宗师】%s：%s"NOR"\n",
			"msg_emote": HIC "【塞外宗师】%s" NOR"\n",
			"msg_color": HIC,
			"name"     : "塞外宗师",
			"for_listen": 1,
		]),
	"jiaoyi":([	"msg_speak": HIC "【拍卖】%s：%s"NOR"\n",
			"msg_color": HIC,
			"name"     : "拍卖场",
		]),
]);
...
varargs int do_channel(object me, string verb, string arg, int emote)
{
	...
	if (userp(me))
	{
		...
		case "party":
			...
			channels[verb]["msg_speak"] = HIC "【" + party +
						      "】%s：%s"NOR"\n";
			channels[verb]["msg_emote"] = HIC "【" + party +
						      "】%s" NOR"\n";
			break;
		...
		case "family":
			...
			channels[verb]["msg_speak"] = HIC "【" + family +
						      "】%s：%s"NOR"\n";
			channels[verb]["msg_emote"] = HIC "【" + family +
						      "】%s" NOR"\n";
			break;
	...
	if (is_player || ! stringp(who = me->query("channel_id")))
	{
		...
		if (who == me->name(1))
		{
			...
			svip = (me->query("chenghao")?(svip+"("+me->query("chenghao") +")"):"");

			who = channels[verb]["msg_color"]+ svip+ ZJURL("cmds:look "+me->query("id"))+ZJSIZE(22)+who+NOR;
		}
//			who += channels[verb]["msg_color"] + "[" + capitalize(me->query("id")) + "]";
		if (who[0] == 27) who = NOR + who;
		who += channels[verb]["msg_color"];
	}
// 	else if (playerp(me) || !stringp(who = me->query("channel_id")))
// 	{
// 		string ys,svip;
// 		int v;
...
// 				   "零":HIW,
// ]);
// 			titles = TITLE_D->alltitles();
// 			titles = titles[me->query("chenghao")];
// 			if (titles["reputation"])
// 				tlevel = titles["reputation"] / 200000;
// 			else
// 				tlevel = 0;
// 			if (tlevel > 5)
// 				tlevel = 5;
// 			size = 25 - tlevel;
// 			svip = sprintf(yanshe[chinese_number(tlevel)] + "%s%s)" NOR, "(", me->query("chenghao"));
// 		}
// 		else
// 			svip = "";
// 		who = me->name(channels[verb]["name_raw"]);
...
// 			else
// 				svip += HIC "♀" NOR;
// 			who = channels[verb]["msg_color"] + svip + ZJURL("cmds:look " + me->query("id")) + who + NOR;
// 		}
